 ...
 * \author Chen Zhu
*/
#ifndef MXNET_OPERATOR_FFT_INL_H
#define MXNET_OPERATOR_FFT_INL_H
...
public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    for (size_t idx=0; idx<num_compute; ++idx) {
      complex_data = complex_pad_imag(data.Slice(idx*param_.compute_size, idx*param_.compute_size+param_.compute_size));
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    // In this solution, out_grad must comes from a fft of real signal, so that it is Hermitian symmetric, giving a real output
    ...
    for (size_t idx=0; idx<num_compute; ++idx) {
      ...
      Assign(gdata.Slice(idx*param_.compute_size, (idx+1)*param_.compute_size), req[fft::kData], complex_toreal(complex_data));
    }
    cufftDestroy(plan);
    ...
    if (remain_num>0){
      ...
      Assign(gdata.Slice(param_.compute_size*num_compute, param_.compute_size*num_compute+remain_num), req[fft::kData], complex_toreal(complex_data));
      cufftDestroy(plan_remain);    
    }
